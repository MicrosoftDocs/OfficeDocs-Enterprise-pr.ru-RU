---
title: 'Общие сведения: разделенный туннеллинг VPN с Office 365'
ms.author: kvice
...
- Может быстро настраиваться, тестироваться и внедряться заказчиками без дополнительных требований к инфраструктуре или приложениям.
  ...
  В зависимости от платформы VPN и сетевой архитектуры, внедрение может занять всего несколько часов. Для получения дополнительной информации см. [Внедрение VPN-туннелирования](office-365-vpn-implement-split-tunnel.md#implement-vpn-split-tunnelling).
...
Полное руководство по внедрению см. в разделе [Внедрение раздельного туннелирования VPN для Office 365](office-365-vpn-implement-split-tunnel.md).
...
![Принудительная настройка VPN](media/vpn-split-tunnelling/vpn-model-1.png)
...
Office 365 классифицирует необходимые конечные точки для Office 365 на три категории: **оптимизация****разрешить** и **по умолчанию**. Конечные точки с меткой **Оптимизация** находятся в центре нашего внимания и имеют следующие характеристики:
...
Этот ограниченный набор конечных точек можно выделить из принудительного VPN-туннеля и безопасно и напрямую отправить в службу Office 365 через локальный интерфейс пользователя. Это известно как **разделенное туннелирование**.
...
[Внедрение раздельного VPN-туннелирования для Office 365](office-365-vpn-implement-split-tunnel.md)
...
[Инструмент сетевого подключения Office 365](https://aka.ms/netonboard)
