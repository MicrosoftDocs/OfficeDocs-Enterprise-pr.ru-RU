...
Exchange Online синхронизирует свойство **PreferredDataLocation** из Azure AD в свойство **MailboxRegion** в службе каталогов Exchange Online. Значение **MailboxRegion** определяет географически, где будет помещен почтовых ящиков пользователей и все связанные архивные почтовые ящики. Настройка основного почтового ящика и архивирование почтовых ящиков пользователей могут располагаться в разных Geos невозможна. Только один географически можно настроить для каждого объекта-пользователя.

- При настройке **PreferredDataLocation** на пользователя с помощью существующего почтового ящика почтового ящика будут помещены в очередь перемещения и автоматическое перемещение в указанном географически. 
...
- При **PreferredDataLocation** не указан на пользователя, почтовый ящик будет помещен в географически по умолчанию.

- Если код **PreferredDataLocation** неверен (например тип NAN вместо им), почтовый ящик будет помещен в географически по умолчанию.
...
1. Ферма с несколькими географически возможности поддержки только почтовые ящики пользователей, почтовые ящики ресурса (почтовые ящики оборудования и помещений) и общие почтовые ящики. Общедоступные папки почтовых ящиков и групп Office 365 оставаться в географически Домашняя страница клиента.
...
> **Примечание:** Если код Geo в поле имя базы данных не соответствует значение **MailboxRegion** , почтовый ящик будет автоматически быть помещены в очередь перемещения и перемещено в географически, указанное значение **MailboxRegion** (Exchange Online ищет несоответствие между этими значения свойств).
...
- Сколько времени занимает о перемещении mailboxfrom его текущего географически для нового желаемую географически зависит от нескольких факторов:
