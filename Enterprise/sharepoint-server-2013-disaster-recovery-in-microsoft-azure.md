...
[![Процесс аварийного восстановления SharePoint в Azure](images/SP_DR_Azure.png)
...
Следуйте указаниям из статьи [Архитектуры Microsoft Azure для SharePoint 2013](microsoft-azure-architectures-for-sharepoint-2013.md), чтобы разработать среду аварийного восстановления, включая ферму восстановления SharePoint. Чтобы приступить к разработке, вы можете использовать иллюстрации из файла Visio [Решение для аварийного восстановления SharePoint в Azure](https://go.microsoft.com/fwlink/p/?LinkId=392554). Рекомендуем полностью разработать среду, прежде чем приступать к работе в среде Azure.
...
Для внешнего доступа к ферме SharePoint можно создать на внешнем DNS-сервере с URL-адресом, который клиенты используют в интрасети (например, http://sharepoint.contoso.com) запись узла, которая указывает на внешний IP-адрес в брандмауэре (в этом примере рекомендуется настроить разделенный DNS, чтобы внутренний DNS-сервер был полномочным сервером для домена contoso.com и направлял запросы непосредственно в кластер фермы SharePoint, а не на внешний DNS-сервер). Затем можно сопоставить внешний IP-адрес со внутренним IP-адресом локального кластера, чтобы клиенты нашли нужные им ресурсы.
  
После этого может возникнуть один из следующих сценариев аварийного восстановления.
...
|:-----|:-----|:-----|:-----|
|C  <br/> |80  <br/> |Системный диск  <br/> |<DriveLetter>:\\Файлы программ\\Microsoft SQL Server\\  <br/> |
|E  <br/> |80  <br/> |Диск журналов (40 ГБ)  <br/> |<DriveLetter>:\\Файлы программ\\Microsoft SQL Server\\MSSQL10_50.MSSQLSERVER\\MSSQL\\данных  <br/> |
|F  <br/> |80  <br/> |Диск файла подкачки (36 ГБ)  <br/> |<DriveLetter>:\\Файлы программ\\Microsoft SQL Server\\MSSQL\\данных  <br/> |
...
|**Буква диска**|**Размер**|**Имя каталога**|**Путь**|
|:-----|:-----|:-----|:-----|
|C  <br/> |80  <br/> |Корневой каталог данных  <br/> |<DriveLetter>:\\Файлы программ\\Microsoft SQL Server\\  <br/> |
|E  <br/> |500  <br/> |Каталог базы данных пользователей  <br/> |<DriveLetter>:\\Файлы программ\\Microsoft SQL Server\\MSSQL10_50.MSSQLSERVER\\MSSQL\\данных  <br/> |
|F  <br/> |500  <br/> |Каталог журнала базы данных пользователей  <br/> |<DriveLetter>:\\Файлы программ\\Microsoft SQL Server\\MSSQL10_50.MSSQLSERVER\\MSSQL\\данных  <br/> |
|G  <br/> |500  <br/> |Временный каталог БД  <br/> |<DriveLetter>:\\Файлы программ\\Microsoft SQL Server\\MSSQL10_50.MSSQLSERVER\\MSSQL\\данных  <br/> |
|H  <br/> |500  <br/> |Временный каталог журнала БД  <br/> |<DriveLetter>:\\Файлы программ\\Microsoft SQL Server\\MSSQL10_50.MSSQLSERVER\\MSSQL\\данных  <br/> |
...
Мы использовали параметр командной строки  _skipRegisterAsDistributedCachehost_ при запуске программы **psconfig.exe**. Дополнительные сведения см. в статье[Планирование веб-каналов и службы распределенного кэша в SharePoint Server 2013](https://go.microsoft.com/fwlink/p/?linkid=270985). 
...
Проверка отсутствующих связи приложения службы между семейства веб-сайтов контента и вашей концентратора типов контента. Кроме того, в разделе **управляемых метаданных - <site collection name> подключения** свойства экрана, убедитесь, что этот параметр включен: **этого приложения-службы — это место хранения по умолчанию для наборов терминов, определяемых столбца.**
...
### <a name="availability-group-creation-fails-at-starting-the-alwaysonhealth-xevent-session-on-server-name"></a>Ошибка создания группы доступности на этапе запуска сеанса XEvent "AlwaysOn_health" на сервере <имя сервера>
...
[Поддерживаемые варианты обеспечения высокого уровня доступности и аварийного восстановления для баз данных SharePoint (SharePoint 2013)](https://go.microsoft.com/fwlink/p/?LinkId=393121)
