...
Используйте рекомендации [Архитектуры Microsoft Azure для SharePoint 2013](microsoft-azure-architectures-for-sharepoint-2013.md) для разработки среды аварийного восстановления, включая восстановления фермы SharePoint. Графические объекты можно использовать в файле Visio [SharePoint решения аварийного восстановления в среде Azure](https://go.microsoft.com/fwlink/p/?LinkId=392554) для начала процесса разработки. Рекомендуется разрабатывать всей среды перед началом никакой работы в среде Azure.
...
Внешний доступ к ферме SharePoint, можно создать запись узла на внешнем DNS-сервере с помощью же URL-адрес, используемый клиентами в интрасети (например, `http://sharepoint.contoso.com`), который указывает на внешний IP-адрес в брандмауэре. (Рекомендуется с помощью данного примера является настройка разделения DNS, чтобы внутренний DNS-сервер отвечает за `contoso.com` и передает запросы непосредственно к кластеру фермы SharePoint, а не маршрутизации DNS запросы внешних DNS-сервера.) Затем можно сопоставить внешний IP-адрес внутреннего IP-адрес локального кластера, чтобы найти ресурсы, которые они постоянно ищут клиентов.
...
При использовании именованных семейств сайтов, соответствии с рекомендациями в [семейство сайтов с именем узла архитектура и развертывание (SharePoint 2013)](https://docs.microsoft.com/SharePoint/administration/host-named-site-collection-architecture-and-deployment)могут иметь несколько семейств сайтов, размещенных же веб-приложении в ферме SharePoint с уникальным DNS-имен (например, `http://sales.contoso.com` и `http://marketing.contoso.com`). В этом случае можно создать DNS-записей для каждого семейства веб-сайтов, указывающие на IP-адреса кластера. После запроса достигает серверы веб интерфейс SharePoint, обработки маршрутизации каждого запроса к коллекции соответствующих веб-сайтов.
...
Мы использовали параметр _skipRegisterAsDistributedCachehost_ , когда мы выполнили **psconfig.exe** из командной строки. Для получения дополнительных сведений см. [Планирование веб-каналов и службы распределенного кэша в SharePoint Server 2013](https://docs.microsoft.com/sharepoint/administration/plan-for-feeds-and-the-distributed-cache-service). 
...
Службы может занять несколько минут, чтобы начать, в зависимости от производительности и текущей нагрузки SharePoint Server. Нажмите кнопку **Пуск** для службы и предоставляют достаточно времени для запуска при обновлении служб на экране сервера для отслеживания состояния иногда вручную. В случае, если служба остается остановлена, включить сбор данных диагностики SharePoint, попытайтесь снова запустить службу и проверьте журнал на наличие ошибок. Дополнительные сведения в статье [Configure диагностических данных в SharePoint 2013](https://docs.microsoft.com/sharepoint/administration/configure-diagnostic-logging)
