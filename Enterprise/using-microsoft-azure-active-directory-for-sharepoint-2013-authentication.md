...
# Использование службы Microsoft Azure Active Directory для проверки подлинности в SharePoint 2013
...
## Создание клиента и пространства имен Azure AD
...
## Добавление поставщика удостоверений WS-Federation к пространству имен
...
## Добавление SharePoint в качестве приложения проверяющей стороны
...
1. На портале Azure Access Control выберите пункт **Приложения проверяющей стороны** и нажмите кнопку **Добавить**, как показано на следующем рисунке.
     ...
     ![Диаграмма с параметрами приложения проверяющей стороны](images/RelyingPartyApplications.jpg)
  
## Создание самозаверяющего сертификата для SSL
...
## Создание группы правил для проверки подлинности на основе утверждений
...
## Создание сопоставления утверждений с помощью Windows PowerShell
...
## Настройка SharePoint для нового поставщика удостоверений

Выполните указанные ниже действия, чтобы настроить свою установку SharePoint для нового поставщика удостоверений Azure AD.
  
1. Убедитесь, что учетная запись пользователя, который выполняет эту процедуру, является участником группы администраторов фермы SharePoint.
