...
>- Обзор использования раздельного VPN-туннелирования для оптимизации подключения Office 365 для удаленных пользователей см. в статье [Обзор: раздельное VPN-туннелирование для Office 365](office-365-vpn-split-tunnel.md).
...
>[!NOTE]
>Текущее требование для того, чтобы это работало в 100% сценариев, также должно добавить диапазон **13.107.60.1/32**. В скором времени эта необходимость отпадет благодаря обновлению в клиенте Teams, выход которого запланирован на **июнь 2020 года**. Мы обновим эту статью сведениями о сборке по мере поступления этой информации.
...
- Простой tracert к конечной точке в пределах области разделенного туннеля должен показать выбранный путь, например:
  ...
  Затем вы увидите путь через локального поставщика услуг Интернета к этой конечной точке, которая должна быть разрешена в IP-адрес в диапазонах Teams, настроенных для раздельного туннелирования.
...
[Обзор: раздельное VPN-туннелирование для Office 365](office-365-vpn-split-tunnel.md)
