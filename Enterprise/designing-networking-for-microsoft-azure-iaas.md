...
Выполните в разделе **действия по подготовке сети для Microsoft облачных служб** в [Общие элементы Microsoft cloud подключения](common-elements-of-microsoft-cloud-connectivity.md).
  
### <a name="step-2-optimize-your-internet-bandwidth"></a>Шаг 2. Оптимизируйте пропускную способность канала доступа в Интернет.

Оптимизация вашей пропускной способности сети, выполнив шаги 2 – 4 в разделе **действия по подготовке сети для служб Microsoft SaaS** разработки [сети для Microsoft SaaS](designing-networking-for-microsoft-saas.md).
...
**На рисунке 1: Только в облаке VNet**
...
**На рисунке 2: Распределенной VNet**
...
Просмотреть дополнительные [этапы планирования для распределенной VNet](designing-networking-for-microsoft-azure-iaas.md#cross_prem) раздел в этой статье.
...
|**Тип VNet**|**Адресное пространство виртуальной сети**|
|:-----|:-----|
 ...
 **Таблица 1: Типы VNets и соответствующие им адресное пространство**
...
**На рисунке 3: Два типа подсети в Azure**

![Рис. 3. Два типа подсетей в Azure](images/2eaa512d-1293-4e9b-b927-6bfe0fc0acb4.png)
  
На рисунке 3 представлена виртуальная сеть с подсетью шлюза, в которой имеется шлюз Azure, и набором подсетей с размещенными виртуальными машинами.
...
Ниже представлены рекомендации по определению адресного пространства для подсети шлюза Azure.
...
|**Обязательный виртуальных машин**|**Бит узла**|**Размер подсети**|
|:-----|:-----|:-----|
|1-3  <br/> |3  <br/> |/29  <br/> |
|4-11  <br/> |4  <br/> |/28  <br/> |
|12-27  <br/> |5  <br/> |/27  <br/> |
 ...
 **В таблице 3: требования к виртуальной машины и их размеров подсети**
 ...
 **В таблице 4: Параметры сервера DNS для двух различных типов VNets**
...
**На рисунке 4: Балансировки нагрузки в Azure**
...
**На рисунке 5: Внутренний подсистемы балансировки нагрузки в Azure**
...
**На рисунке 6: Виртуальных устройств и пользовательских маршрутов в Azure**
...
|4. сетевых устройств безопасности в Azure 
 Магазин (не отображается)  <br/> |Классическая с использованием диспетчера ресурсов  <br/> |
   
 **В таблице 5: Способа подключения к виртуальных машин и соответствующих моделей Azure развертывания**
  
**На рисунке 7: Подключение к виртуальных машин Azure в Интернете**
...
**На рисунке 8: Последовательно конфигурацию для VNets**
...
**На рисунке 9: Звездообразная и сервер-концентратор конфигурацию для VNets**
...
**На рисунке 10: Полной сетки конфигурации для VNets**
...
> [!TIP]
> Создание имитации распределенной среде разработки и тестирования, видеть [модель распределенной виртуальной сети в Azure](simulated-cross-premises-virtual-network-in-azure.md). 
 ...
 **В таблице 6: Типы подключений для VNets между организациями**
...
**На рисунке 11: Четыре способа подключения к VNet между организациями**
...
**На рисунке 12: Маршрутизатор VPN в локальной или устройство**
...
**На рисунке 13: Локальные маршруты, необходимые для VNet доступен**
...
**На рисунке 14: Использование ExpressRoute для подключения к VNet между организациями**
...
**Рисунок 15: Локальную сеть адресное пространство для VNet между организациями**
...
Ниже перечислены способы определения адресного пространства локальной сети.
...
**На рисунке 16: Адрес места отверстия созданные VNet адресное пространство**
...
|2. список префиксов без перекрытия для переменной октетов до, но не включая последний используется 
 октета в VNet адресного пространства.  <br/> |10.0.0.0/16, 10.1.0.0/16... 10.99.0.0/16, 10.101.0.0/16... 10.254.0.0/16 10.255.0.0/16 (255 префиксов, пропуск 10.100.0.0/16)  <br/> |
|3. без перекрытия приставок в список 
 Последний использованный октета VNet адресного пространства.  <br/> | 10.100.0.0/24, 10.100.1.0/24... 10.100.99.0/24, 10.100.101.0/24... 10.100.254.0/24 10.100.0.255.0/24 (255 префиксов, пропуск 10.100.100.0/24)  <br/> |
   
 **В таблице 7: Пример локальной сети пространства имен**
...
**Рисунке 17: Репликация DNS и пересылки для DNS-сервер между организациями VNet**
...
**На рисунке 18: Пользовательские маршрутов и Принудительное туннелирование для VNet между организациями**
...
На рисунке 19 показан пример ИТ-нагрузки в интрасети, размещенной в Azure IaaS (высокодоступная многоуровневая ферма SharePoint Server 2016).
  
**На рисунке 19: Высокой доступностью интрасети SharePoint Server 2016 фермы в Azure IaaS**

![Высокодоступная ферма SharePoint Server 2016 в Azure IaaS](images/3a922e21-df91-455f-ba90-78abdd48d98d.png)
  
На рисунке 19 показан девяти серверов SharePoint Server 2016 фермы, развернутой в VNet между организациями, поддерживающей подсистемы балансировки нагрузки внутренних уровни переднего плана и данных. Дополнительные сведения, включая пошаговые проектирования и инструкции по развертыванию можно [2016 сервера SharePoint в Microsoft Azure](https://technet.microsoft.com/library/mt779107%28v=office.16%29.aspx).
  
> [!TIP]
> Создание фермы SharePoint Server 2016 одним сервером в VNet имитации между организациями, видеть [интрасети 2016 сервера SharePoint в среде Azure разработку и тестирование](https://technet.microsoft.com/library/mt806351%28v=office.16%29.aspx). 
  
Дополнительные примеры рабочих нагрузок ИТ, развернутых на виртуальных машинах в виртуальной среде Azure распределенной сети [гибридных сценариев облака для Azure IaaS](https://technet.microsoft.com/library/mt750502.aspx)см.
