 ...
 **Сводка:** Узнайте, как использовать Windows PowerShell для выполнения миграции IMAP в Office 365.
...
Если миграции электронной почты от реализации IMAP для Microsoft Exchange используйте формате **Домен/Admin_UserName/User_UserName** для атрибута **имени пользователя** в CSV-файл. Предположим, что в случае миграции электронной почты из Exchange для Терри Адамса, Анна Белова и Paul Cannon. У вас есть почты учетной записью администратора, где **mailadmin** — это имя пользователя и пароль **P@ssw0rd**. Вот, как будет выглядеть CSV-файла:
