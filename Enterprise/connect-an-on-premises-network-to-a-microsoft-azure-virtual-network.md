...
Распределенной виртуальная сеть Azure подключен к локальной сети, расширение сеть, чтобы включать подсети и виртуальных машин, размещенных в службах инфраструктуры. Это подключение позволяет компьютерам в локальной сети для прямого доступа к виртуальных машин в Azure и наоборот. 

Например сервер синхронизации каталогов, работающим на Azure виртуальной машины должен запроса контроллеры домена в локальной для изменения учетных записей и синхронизировать эти изменения с подписки Office 365. В этой статье показано, как настроить виртуальный Azure распределенной сети с помощью подключения виртуальной частной сети сеть сеть (VPN), который будет готов для размещения виртуальных машин Azure.
...
На схеме есть две сети, подключенных по VPN-подключения веб сайта: в локальной сети и виртуальная сеть Azure. VPN-подключение веб сайта — это:

- Между двумя конечными точками, которые адресации и расположены на общедоступную сеть Интернет.
- Завершен, устройство VPN в локальной сети и шлюз Azure VPN в Azure виртуальной сети.

Виртуальная сеть Azure служит для размещения виртуальных машин. Сетевой трафик от виртуальных машин в Azure виртуальной сети получает переадресовываться шлюз виртуальной частной сети, который затем перенаправляет трафик через VPN-подключение веб сайте на устройство VPN в локальной сети. Затем инфраструктуры маршрутизации в локальной сети перенаправляет трафик в место назначения.

>[!Note]
>Также можно использовать [ExpressRoute](https://azure.microsoft.com/services/expressroute/), которая является прямое подключение между вашей организации и сети корпорации Майкрософт. Трафик через ExpressRoute не передаются через общедоступную сеть Интернет. В этой статье не рассматриваются использование ExpressRoute.
>
...
2. **Microsoft Azure:** Создайте Azure виртуальной сети с помощью VPN-подключения веб сайта. 
...
- Azure подписки. Сведения о Azure подписок перейдите на [страницу как для приобрести Azure](https://azure.microsoft.com/pricing/purchase-options/).
...
Создание виртуальных машин, необходимую в Azure. Для получения дополнительных сведений см. [Создание виртуальной машины Windows Azure портала](https://go.microsoft.com/fwlink/p/?LinkId=393098).
