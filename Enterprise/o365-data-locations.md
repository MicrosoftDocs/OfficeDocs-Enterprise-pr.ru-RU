---
title: Расположения данных Майкрософт 365
ms.author: deanw
author: dmwmsft
manager: laurawi
ms.date: 06/17/2020
audience: ITPro
ms.topic: hub-page
ms.service: o365-solutions
localization_priority: Priority
search.appverid:
- MET150
ms.collection:
- Ent_O365
- Strat_O365_Enterprise
ms.custom:
- Ent_TLGs
ms.assetid: 706d5449-45e5-4b0c-a012-ab60501899ad
description: 'Сводка: расположения данных Microsoft 365'
ms.openlocfilehash: a0f62fa7955224c07c8e5af35fb53229e3aeb6c4
ms.sourcegitcommit: e0603d9259e9be1aeb110fc3dd8b9abc6138c55d
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 06/17/2020
ms.locfileid: "44781064"
---
<!--
Note to new owner: Links in the Notes and FAQ sections need to be added from the source document at https://microsoft.sharepoint.com/:w:/t/golocalteam/Ebv6_Lw6FvNKucfA1eESTPUBy6O1MdvWEeT-L8O5a2HsIg?e=4%3AUfF1Uj&at=9&xsdata=&sdata=MkxOUXhlZEdyR0I1SzBvd044d2hkd2RPaGN2RUpaS09jMkZTcys0dkZQRT0%3D
-->

# <a name="where-your-microsoft-365-customer-data-is-stored"></a>Где хранятся данные клиента Microsoft 365

В таблицах ниже показано, где хранятся данные клиентов для служб Microsoft 365 в глобальных облачных расположениях Майкрософт. Разверните расположение своей страны выставления счетов, чтобы узнать, где будут храниться данные клиента для каждой службы.

Пользователям следует просмотреть сведения о расположении данных для конкретных клиентов в вашем Центре администрирования Office 365 в разделе **Параметры | Профиль организации | Карточка расположения данных**.

Новые клиенты Microsoft 365 по умолчанию основаны на стране транзакции, связанной с первой подпиской этого клиента.

Сведения о контрактных обязательствах по расположению хранилища неактивных данных клиента см. в [условиях использования Microsoft Online Services (OST)](https://go.microsoft.com/fwlink/p/?LinkId=2098215).

Расположения данных Azure Active Directory см. на [карте данных AAD](https://go.microsoft.com/fwlink/p/?linkid=2092972).


Из-за неприоритетных обстоятельств в КОВИД-19 и необходимости управлять спросом на веб-службы в Европе, если ваша организация является образовательным учреждением, мы можем подготовить свой клиент Microsoft 365 в Европейского союза (ЕС), Европейской связи Европейского союза (ЕАСТ), Великобритании (США), США (США) или Канаде (CA) или передавать данные в любые центры обработки данных в ЕС , ЕАСТ, Великобритании, США или CA.  Это означает, что ваши данные могут храниться в ЕС, ЕАСТ, Соединенном Королевстве, США или Канаде.  Наши обязательства по обеспечению соответствия требованиям, конфиденциальности и безопасности остаются неизменными при реализации необходимых и рациональных мер по обеспечению доступности служб.  Мы соблюдаем все соответствующие законы в отношении передачи данных.




## <a name="faq"></a>Вопросы и ответы

### <a name="how-does-microsoft-define-data"></a>Как Майкрософт определяет данные?

В центре управления безопасностью Майкрософт просмотрите [определения для различных типов данных](https://go.microsoft.com/fwlink/p/?linkid=864390). В [условиях использования Microsoft Online Services](https://go.microsoft.com/fwlink/p/?linkid=862403) корпорация Майкрософт принимает на себя контрактные обязательства в отношении данных клиента и ваших данных. Под основными данными клиента мы понимаем данные клиента, которые принимаются на хранение только в вашем регионе согласно [условиям использования Microsoft Online Services](https://go.microsoft.com/fwlink/p/?linkid=862403).

### <a name="where-are-the-exact-addresses-of-the-data-centers"></a>Каковы точные адреса центров обработки данных?

Корпорация Майкрософт не раскрывает точные адреса своих центров обработки данных. Мы установили эту политику, чтобы защитить наши центры обработки данных.  Однако мы указываем расположения по городам. Дополнительные сведения см. в статье [расположения центров обработки данных](https://docs.microsoft.com/office365/enterprise/o365-data-locations#data-center-locations).

### <a name="does-the-location-of-your-customer-data-have-a-direct-impact-on-your-end-users-experience"></a>Оказывает ли расположение данных клиента непосредственное влияние на работу конечных пользователей?

Производительность Microsoft 365 не просто пропорциональна расстоянию пользователя между расположениями центра обработки данных. Непрерывные инвестиции корпорации Майкрософт в глобальной облачной сети, глобальной облачной инфраструктуре и архитектуре служб Microsoft 365 помогают обеспечить пользователям единственного, согласованного взаимодействия независимо от того, где хранятся данные клиента. Если у ваших пользователей возникают проблемы с производительностью, вам следует тщательно устранять их. Корпорация Майкрософт опубликовала рекомендации по планированию и оптимизации производительности конечных пользователей на [веб-сайте поддержки Office](https://go.microsoft.com/fwlink/p/?linkid=862645)365 для пользователей Microsoft.

### <a name="how-does-microsoft-help-me-comply-with-my-national-regional-and-industry-specific-regulations"></a>Как Майкрософт помогает обеспечить соответствие национальным, региональным и отраслевым нормам?

Чтобы обеспечить соблюдение требований национальных, региональных и отраслевых требований, регулирующих сбор и использование данных пользователей, Microsoft 365 предлагает наиболее полный набор предложений по обеспечению соответствия требованиям любого глобального облачного поставщика. Ознакомьтесь с [нашими нашими требованиями](https://go.microsoft.com/fwlink/p/?linkid=864391) и дополнительными сведениями в разделе [Microsoft 365 соответствие требованиям](https://go.microsoft.com/fwlink/p/?linkid=862317) в центре управления безопасностью (Майкрософт). Кроме того, некоторые планы Microsoft 365 предоставляют дальнейшие решения по обеспечению соответствия требованиям для управления данными, соблюдения законодательных и нормативных требований, а также отслеживания действий, выполняемых над данными.

### <a name="who-can-access-your-data-and-according-to-what-rules"></a>У кого есть доступ к данным и в соответствии с какими правилами?

 Корпорация Майкрософт применяет надежные меры по защите данных клиента от несанкционированного доступа и использования неавторизованными лицами. К ним относится ограничение доступа сотрудниками и субподрядчиками Майкрософт, а также тщательное определение требований к реагированию на запросы государственных органов на получение данных клиента. Однако вы можете в любое время получить доступ к собственным данным клиента по любой причине. Дополнительные сведения см. в [центре управления безопасностью Майкрософт](https://go.microsoft.com/fwlink/p/?linkid=864392).

### <a name="does-microsoft-access-your-data"></a>Обращается ли Майкрософт к вашим данным?

Корпорация Майкрософт автоматизирует большинство операций Microsoft 365, предоставляя собственный доступ к данным клиентов. Это помогает нам управлять Microsoft 365 на масштабируемости и устранять риски внутренних угроз для данных клиентов. По умолчанию у специалистов Майкрософт нет прав администратора и нет доступа к данным клиентов в Microsoft 365. Инженеры корпорации Майкрософт могут получать ограниченный регистрируемый доступ к данным клиента в течение ограниченного времени, но только при необходимости выполнения обычных операций обслуживания и только с утверждения сотрудником старшего руководства корпорации Майкрософт (а для клиентов с лицензией на функцию защищенного хранилища — с утверждения клиентом).

### <a name="how-does-microsoft-secure-your-data"></a>Как Майкрософт защищает ваши данные?

Корпорация Майкрософт обладает надежными политиками, элементами управления и системами, встроенными в Microsoft 365, чтобы обеспечить безопасность информации. Ознакомьтесь с [разделом безопасность microsoft 365](https://go.microsoft.com/fwlink/p/?linkid=864393) в центре управления безопасностью, чтобы узнать больше.

### <a name="does-microsoft-365-encrypt-your-data"></a>Шифрует ли данные Microsoft 365?

Microsoft 365 использует технологии на стороне службы, которые шифруют данные клиентов в REST и в транзитном месте. Для клиентских данных в REST Microsoft 365 использует шифрование на уровне тома и на уровне файлов. Для данных клиентов в транзите Microsoft 365 использует несколько технологий шифрования для обмена данными между центрами обработки данных и между клиентами и серверами, такими как протокол TLS и безопасность протокола IP (IPsec). Microsoft 365 также включает функции шифрования, управляемые пользователем.

### <a name="where-can-i-find-data-residency-information-for-microsoft-azure"></a>Где можно найти сведения о размещении данных для Microsoft Azure?

Сведения о размещении данных для Microsoft Azure см. на странице [Доступность продуктов по регионам](https://go.microsoft.com/fwlink/p/?linkid=2093451).

### <a name="what-are-multi-geo-capabilities-in-microsoft-365"></a>Что такое поддержка нескольких регионов в Microsoft 365?

[Поддержка нескольких регионов](https://go.microsoft.com/fwlink/p/?linkid=872033) дает клиентам возможность распределять основные данные клиентов по всем доступным расположениям в Microsoft 365 Cloud.   

### <a name="what-services-support-multi-geo"></a>В каких службах есть функция поддержки нескольких регионов?

[Поддержка нескольких регионов](https://go.microsoft.com/fwlink/p/?linkid=872033) доступна в Exchange Online, OneDrive и SharePoint Online. Корпорация Майкрософт изучает поддержку нескольких регионов для других служб Microsoft 365.

### <a name="what-are-the-exceptions-for-intune-data-locations"></a>Каковы исключения для расположений данных Intune?

Корпорация Майкрософт не будет хранить данные клиентов Intune на месте, расположенном вне указанного географического географа. Кроме следующих:

- Это необходимо для того, чтобы корпорация Майкрософт предоставляла поддержку клиентов, устранять неполадки в службе или соответствовать юридическим требованиям.
- Клиент настраивает учетную запись, чтобы включить такое хранение данных клиента, в том числе с помощью следующих компонентов:

  - Функции, предназначенные для использования на глобальном уровне, например сеть доставки содержимого (CDN), которая обеспечивает глобальную службу кэширования и сохраняет данные клиентов в расположениях по всему миру.

  - Azure Active Directory (Azure AD), которая может хранить данные каталога глобально. Это не относится к развертываниям Azure AD в Соединенных Штатах Америки (в которых данные Azure AD хранятся исключительно в США) или в Европе (где данные Azure AD хранятся в Европе или США).
   
  - Предварительная версия, бета-версия или другие услуги предварительной версии, которые обычно хранят данные клиентов в США, но могут хранить их глобально.

Независимо от того, Майкрософт не контролирует и не ограничивает географическое значение, от которого клиенты или их пользователи могут получать доступ к данным клиента. 

## <a name="data-center-locations"></a>Расположения центров обработки данных

В таблице ниже определены расположения данных для различных служб. При определении расположений служб используйте эти таблицы.

### <a name="data-center-geographies"></a>Географические регионы центров обработки данных

В следующих глобальных регионах могут храниться неактивные данные. Расположения, в которых можно хранить данные клиента, могут изменяться. 

| Глобальные географические регионы | Расположения, где могут храниться данные клиента |
| --- | --- |
| Глобальный географический регион 1 — EMEA (Европа, Ближний Восток и Африка) | Австрия, Финляндия, Франция, Ирландия, Нидерланды |
| Глобальный географический регион 2 — Азиатско-Тихоокеанский регион | Гонконг (САР), Япония, Малайзия, Сингапур, Южная Корея |
| Глобальный географический регион 3 — Северная и Южная Америка | Бразилия, Чили, Соединенные Штаты |

## <a name="countryregion-specific-data-center-city-locations"></a>Расположения городов центров обработки данных по странам и регионам

Для центров обработки данных отдельных стран или регионов в следующей таблице определены города, в которых хранятся неактивные данные клиента.

| Страна | Город |
| --- | --- |
| Австралия | Сидней, Мельбурн |
| Канада | г. Квебек, Торонто |
| Европейский Союз | Австрия (Вена), Финляндия (Хельсинки), Франция (Париж, Марсеилле), Ирландия (Дублин), Нидерланды (Амстердам) |
| Франция | Париж, Марсель |
| Германия | Франкфурт, Берлин |
| Индия | Ченнаи, Мумбаи, Пуна |
| Япония | Осака, Токио, Сайтама |
| Южная Корея | Бусан, Сеул |
| Норвегия | Осло, Ставангер |
| Южная Африка | Кейптаун, Йоханнесбург |
| Швейцария | Женева, Цюрих |
| Объединенные Арабские Эмираты | Дубай, Абу Даби |
| Соединенное Королевство | Дарем, Лондон, Кардифф |
| Соединенные Штаты |  Бойдтон, Шайенн, Чикаго, Де-Мойн, Куинси, Сан-Антонио, Санта-Клара, Сан-Хосе |

Новые клиенты Microsoft 365 по умолчанию основаны на стране адреса выставления счетов, связанном с первой подпиской этого клиента. Чтобы подтвердить место хранения данных клиента для существующих служб Office 365, просмотрите сведения о расположении данных для конкретных клиентов в своем Центре администрирования Office 365 в разделе "Параметры" | "Профиль организации" | Карточка "Расположение данных". Вы также можете развернуть страну, в которой базируется ваш бизнес, в приведенном ниже списке, чтобы узнать, где будут храниться данные клиента, если вы хотите начать использовать службу Microsoft 365 уже сегодня.

## <a name="afghanistan"></a>Афганистан
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online и Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="aland-islands"></a>Аландские острова
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="albania"></a>Албания
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="algeria"></a>Алжир
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="american-samoa"></a>Американское Самоа
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="andorra"></a>Андорра
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="angola"></a>Ангола
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="anguilla"></a>Ангилья
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="antarctica"></a>Антарктида
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online &amp; Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="antigua-and-barbuda"></a>Антигуа и Барбуда
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online &amp; Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="argentina"></a>Аргентина
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 3 — Северная и Южная Америка |
| OneDrive для бизнеса | Глобальный географический регион 3 — Северная и Южная Америка |
| SharePoint Online | Глобальный географический регион 3 — Северная и Южная Америка |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Глобальный географический регион 3 — Северная и Южная Америка |
| EOP | Глобальный географический регион 3 — Северная и Южная Америка |
| Intune | Соединенные Штаты |
| MyAnalytics | Глобальный географический регион 3 — Северная и Южная Америка |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 3 — Северная и Южная Америка |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="armenia"></a>Армения
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="aruba"></a>Аруба
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online &amp; Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="australia"></a>Австралия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Австралия |
| OneDrive для бизнеса | Австралия |
| SharePoint Online | Австралия |
| Skype для бизнеса | Австралия |
| Microsoft Teams | Австралия |
| Office Online и Mobile | Австралия |
| EOP | Австралия |
| Intune | Австралия |
| MyAnalytics | Австралия |
| Планировщик | Австралия |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Австралия |
| Stream | Австралия |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="austria"></a>Австрия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Европейский Союз |
| OneDrive для бизнеса | Европейский Союз |
| SharePoint Online | Европейский Союз |
| Skype для бизнеса | Европейский Союз |
| Microsoft Teams | Европейский Союз |
| Office Online и Mobile | Европейский Союз |
| EOP | Европейский Союз |
| Intune | Европейский Союз |
| MyAnalytics | Европейский Союз |
| Планировщик | Европейский Союз |
| Sway | Соединенные Штаты |
| Yammer | Европейский Союз  |
| Службы OneNote | Европейский Союз |
| Stream | Европейский Союз |
| Доска | Европейский Союз |
| Forms | Европейский Союз |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="azerbaijan"></a>Азербайджан
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик |  Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="bahamas"></a>Багамы
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="bahrain"></a>Бахрейн
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="bangladesh"></a>Бангладеш
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online и Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="barbados"></a>Барбадос
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online &amp; Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="belarus"></a>Беларусь
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="belgium"></a>Бельгия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Европейский Союз |
| OneDrive для бизнеса | Европейский Союз |
| SharePoint Online | Европейский Союз |
| Skype для бизнеса | Европейский Союз |
| Microsoft Teams | Европейский Союз |
| Office Online и Mobile | Европейский Союз |
| EOP | Европейский Союз |
| Intune | Европейский Союз |
| MyAnalytics | Европейский Союз |
| Планировщик | Европейский Союз |
| Sway | Соединенные Штаты |
| Yammer | Европейский Союз  |
| Службы OneNote | Европейский Союз |
| Stream | Европейский Союз |
| Доска | Европейский Союз |
| Forms | Европейский Союз |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="belize"></a>Белиз
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="benin"></a>Бенин
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="bermuda"></a>Бермуды
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online &amp; Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="bhutan"></a>Бутан
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online и Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="bolivia"></a>Боливия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 3 — Северная и Южная Америка |
| OneDrive для бизнеса | Глобальный географический регион 3 — Северная и Южная Америка |
| SharePoint Online | Глобальный географический регион 3 — Северная и Южная Америка |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Глобальный географический регион 3 — Северная и Южная Америка |
| EOP | Глобальный географический регион 3 — Северная и Южная Америка |
| Intune | Соединенные Штаты |
| MyAnalytics | Глобальный географический регион 3 — Северная и Южная Америка |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 3 — Северная и Южная Америка |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="bonaire"></a>Бонэйр
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 3 — Северная и Южная Америка |
| OneDrive для бизнеса | Глобальный географический регион 3 — Северная и Южная Америка |
| SharePoint Online | Глобальный географический регион 3 — Северная и Южная Америка |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Глобальный географический регион 3 — Северная и Южная Америка |
| EOP | Глобальный географический регион 3 — Северная и Южная Америка |
| Intune | Соединенные Штаты |
| MyAnalytics | Глобальный географический регион 3 — Северная и Южная Америка |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 3 — Северная и Южная Америка |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="bosnia"></a>Босния
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик |  Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="botswana"></a>Ботсвана
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="bouvet-island"></a>О-в Буве
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online &amp; Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="brazil"></a>Бразилия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 3 — Северная и Южная Америка |
| OneDrive для бизнеса | Глобальный географический регион 3 — Северная и Южная Америка |
| SharePoint Online | Глобальный географический регион 3 — Северная и Южная Америка |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Глобальный географический регион 3 — Северная и Южная Америка |
| EOP | Глобальный географический регион 3 — Северная и Южная Америка |
| Intune | Соединенные Штаты |
| MyAnalytics | Глобальный географический регион 3 — Северная и Южная Америка |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 3 — Северная и Южная Америка |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="british-indian-ocean-territory"></a>Британская территория в Индийском океане
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="british-virgin-islands"></a>Виргинские о-ва (Британские)
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="brunei"></a>Бруней-Даруссалам
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online и Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="bulgaria"></a>Болгария
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Европейский Союз |
| OneDrive для бизнеса | Европейский Союз |
| SharePoint Online | Европейский Союз |
| Skype для бизнеса | Европейский Союз |
| Microsoft Teams | Европейский Союз |
| Office Online и Mobile | Европейский Союз |
| EOP | Европейский Союз |
| Intune | Европейский Союз |
| MyAnalytics | Европейский Союз |
| Планировщик | Европейский Союз |
| Sway | Соединенные Штаты |
| Yammer | Европейский Союз  |
| Службы OneNote | Европейский Союз |
| Stream | Европейский Союз |
| Доска | Европейский Союз |
| Forms | Европейский Союз |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="burkina-faso"></a>Буркина-Фасо
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="burundi"></a>Бурунди
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="cambodia"></a>Камбоджа
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online &amp; Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="cameroon"></a>Камерун
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="canada"></a>Канада
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Канада |
| OneDrive для бизнеса | Канада |
| SharePoint Online | Канада |
| Skype для бизнеса | Канада |
| Microsoft Teams | Канада |
| Office Online и Mobile | Канада |
| EOP | Канада |
| Intune | Соединенные Штаты |
| MyAnalytics | Канада |
| Планировщик | Канада |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Канада |
| Stream | Канада |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="cape-verde"></a>Кабо-Верде
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="cayman-islands"></a>Острова Кайман
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online &amp; Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="central-african-republic"></a>Центрально-Африканская Республика
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="chad"></a>Чад
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="chile"></a>Чили
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 3 — Северная и Южная Америка |
| OneDrive для бизнеса | Глобальный географический регион 3 — Северная и Южная Америка |
| SharePoint Online | Глобальный географический регион 3 — Северная и Южная Америка |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Глобальный географический регион 3 — Северная и Южная Америка |
| EOP | Глобальный географический регион 3 — Северная и Южная Америка |
| Intune | Соединенные Штаты |
| MyAnalytics | Глобальный географический регион 3 — Северная и Южная Америка |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 3 — Северная и Южная Америка |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="china"></a>Китай
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online и Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="christmas-island"></a>О-в Рождества
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online и Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="cocos-keeling-islands"></a>Кокосовые о-ва
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online и Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="colombia"></a>Колумбия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="comoros"></a>Коморы
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="congo-brazzaville"></a>Конго (Браззавиль)
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="congo-kinshasa"></a>Конго (Киншаса)
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="cook-islands"></a>Острова Кука
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online и Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="costa-rica"></a>Коста-Рика
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="cote-divoire"></a>Кот д'Ивуар
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="croatia"></a>Хорватия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Европейский Союз |
| OneDrive для бизнеса | Европейский Союз |
| SharePoint Online | Европейский Союз |
| Skype для бизнеса | Европейский Союз |
| Microsoft Teams | Европейский Союз |
| Office Online и Mobile | Европейский Союз |
| EOP | Европейский Союз |
| Intune | Европейский Союз |
| MyAnalytics | Европейский Союз |
| Планировщик | Европейский Союз |
| Sway | Соединенные Штаты |
| Yammer | Европейский Союз  |
| Службы OneNote | Европейский Союз |
| Stream | Европейский Союз |
| Доска | Европейский Союз |
| Forms | Европейский Союз |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="curacao"></a>Кюрасао
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 3 — Северная и Южная Америка |
| OneDrive для бизнеса | Глобальный географический регион 3 — Северная и Южная Америка |
| SharePoint Online | Глобальный географический регион 3 — Северная и Южная Америка |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online &amp; Mobile | Глобальный географический регион 3 — Северная и Южная Америка |
| EOP | Глобальный географический регион 3 — Северная и Южная Америка |
| Intune | Соединенные Штаты |
| MyAnalytics | Глобальный географический регион 3 — Северная и Южная Америка |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 3 — Северная и Южная Америка |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="cyprus"></a>Кипр
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Европейский Союз |
| OneDrive для бизнеса | Европейский Союз |
| SharePoint Online | Европейский Союз |
| Skype для бизнеса | Европейский Союз |
| Microsoft Teams | Европейский Союз |
| Office Online и Mobile | Европейский Союз |
| EOP | Европейский Союз |
| Intune | Европейский Союз |
| MyAnalytics | Европейский Союз |
| Планировщик | Европейский Союз |
| Sway | Соединенные Штаты |
| Yammer | Европейский Союз  |
| Службы OneNote | Европейский Союз |
| Stream | Европейский Союз |
| Доска | Европейский Союз |
| Forms | Европейский Союз |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="czech-republic"></a>Чехия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Европейский Союз |
| OneDrive для бизнеса | Европейский Союз |
| SharePoint Online | Европейский Союз |
| Skype для бизнеса | Европейский Союз |
| Microsoft Teams | Европейский Союз |
| Office Online и Mobile | Европейский Союз |
| EOP | Европейский Союз |
| Intune | Европейский Союз |
| MyAnalytics | Европейский Союз |
| Планировщик | Европейский Союз |
| Sway | Соединенные Штаты |
| Yammer | Европейский Союз  |
| Службы OneNote | Европейский Союз |
| Stream | Европейский Союз |
| Доска | Европейский Союз |
| Forms | Европейский Союз |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="denmark"></a>Дания
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Европейский Союз |
| OneDrive для бизнеса | Европейский Союз |
| SharePoint Online | Европейский Союз |
| Skype для бизнеса | Европейский Союз |
| Microsoft Teams | Европейский Союз |
| Office Online и Mobile | Европейский Союз |
| EOP | Европейский Союз |
| Intune | Европейский Союз |
| MyAnalytics | Европейский Союз |
| Планировщик | Европейский Союз |
| Sway | Соединенные Штаты |
| Yammer | Европейский Союз  |
| Службы OneNote | Европейский Союз |
| Stream | Европейский Союз |
| Доска | Европейский Союз |
| Forms | Европейский Союз |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="djibouti"></a>Джибути
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="dominica"></a>Доминика
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="dominican-republic"></a>Доминиканская Республика
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="ecuador"></a>Эквадор
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 3 — Северная и Южная Америка |
| OneDrive для бизнеса | Глобальный географический регион 3 — Северная и Южная Америка |
| SharePoint Online | Глобальный географический регион 3 — Северная и Южная Америка |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Глобальный географический регион 3 — Северная и Южная Америка |
| EOP | Глобальный географический регион 3 — Северная и Южная Америка |
| Intune | Соединенные Штаты |
| MyAnalytics | Глобальный географический регион 3 — Северная и Южная Америка |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 3 — Северная и Южная Америка |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="egypt"></a>Египет
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="el-salvador"></a>Эль-Сальвадор
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="equatorial-guinea"></a>Экваториальная Гвинея
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="eritrea"></a>Эритрея
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="estonia"></a>Эстония
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Европейский Союз |
| OneDrive для бизнеса | Европейский Союз |
| SharePoint Online | Европейский Союз |
| Skype для бизнеса | Европейский Союз |
| Microsoft Teams | Европейский Союз |
| Office Online и Mobile | Европейский Союз |
| EOP | Европейский Союз |
| Intune | Европейский Союз |
| MyAnalytics | Европейский Союз |
| Планировщик | Европейский Союз |
| Sway | Соединенные Штаты |
| Yammer | Европейский Союз  |
| Службы OneNote | Европейский Союз |
| Stream | Европейский Союз |
| Доска | Европейский Союз |
| Forms | Европейский Союз |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="ethiopia"></a>Эфиопия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик |  Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="falkland-islands-malvinas"></a>Фолклендские о-ва (Мальвинские)
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 3 — Северная и Южная Америка |
| OneDrive для бизнеса | Глобальный географический регион 3 — Северная и Южная Америка |
| SharePoint Online | Глобальный географический регион 3 — Северная и Южная Америка |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 3 — Северная и Южная Америка |
| EOP | Глобальный географический регион 3 — Северная и Южная Америка |
| Intune | Соединенные Штаты |
| MyAnalytics | Глобальный географический регион 3 — Северная и Южная Америка |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 3 — Северная и Южная Америка |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="faroe-islands"></a>Фарерские о-ва
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="federated-states-of-micronesia"></a>Федеративные Штаты Микронезии
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online &amp; Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="fiji"></a>Фиджи
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Австралия |
| OneDrive для бизнеса | Австралия |
| SharePoint Online | Австралия |
| Skype для бизнеса | Австралия |
| Microsoft Teams | Австралия |
| Office Online и Mobile | Австралия |
| EOP | Австралия |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Австралия |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Австралия |
| Stream | Австралия |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="finland"></a>Финляндия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Европейский Союз |
| OneDrive для бизнеса | Европейский Союз |
| SharePoint Online | Европейский Союз |
| Skype для бизнеса | Европейский Союз |
| Microsoft Teams | Европейский Союз |
| Office Online и Mobile | Европейский Союз |
| EOP | Европейский Союз |
| Intune | Европейский Союз |
| MyAnalytics | Европейский Союз |
| Планировщик | Европейский Союз |
| Sway | Соединенные Штаты |
| Yammer | Европейский Союз  |
| Службы OneNote | Европейский Союз |
| Stream | Европейский Союз |
| Доска | Европейский Союз |
| Forms | Европейский Союз |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="france"></a>Франция
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Франция |
| OneDrive для бизнеса | Франция |
| SharePoint Online | Франция |
| Skype для бизнеса | Европейский Союз |
| Microsoft Teams | Франция |
| Office Online и Mobile | Франция |
| EOP | Франция |
| Intune | Европейский Союз |
| MyAnalytics | Франция |
| Планировщик | Европейский Союз |
| Sway | Соединенные Штаты |
| Yammer | Европейский Союз |
| Службы OneNote | Франция |
| Stream | Европейский Союз |
| Доска | Европейский Союз |
| Forms | Европейский Союз |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="french-guiana"></a>Французская Гвиана
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 3 — Северная и Южная Америка |
| OneDrive для бизнеса | Глобальный географический регион 3 — Северная и Южная Америка |
| SharePoint Online | Глобальный географический регион 3 — Северная и Южная Америка |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Глобальный географический регион 3 — Северная и Южная Америка |
| EOP | Глобальный географический регион 3 — Северная и Южная Америка |
| Intune | Соединенные Штаты |
| MyAnalytics | Глобальный географический регион 3 — Северная и Южная Америка |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 3 — Северная и Южная Америка |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="french-polynesia"></a>Французская Полинезия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online и Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="french-southern-territories"></a>Французские Южные территории
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online &amp; Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="gabon"></a>Габон
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="gambia"></a>Гамбия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="georgia"></a>Грузия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="germany"></a>Германия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Германия |
| OneDrive для бизнеса | Германия |
| SharePoint Online | Германия |
| Skype для бизнеса | Европейский Союз |
| Microsoft Teams | Германия |
| Office Online и Mobile | Германия |
| EOP | Германия |
| Intune | Европейский Союз |
| MyAnalytics | Германия |
| Планировщик | Европейский Союз |
| Sway | Соединенные Штаты |
| Yammer | Европейский Союз |
| Службы OneNote | Германия |
| Stream | Европейский Союз |
| Доска | Европейский Союз |
| Forms | Европейский Союз |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="ghana"></a>Гана
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="gibraltar"></a>Гибралтар
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="greece"></a>Греция
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Европейский Союз |
| OneDrive для бизнеса | Европейский Союз |
| SharePoint Online | Европейский Союз |
| Skype для бизнеса | Европейский Союз |
| Microsoft Teams | Европейский Союз |
| Office Online и Mobile | Европейский Союз |
| EOP | Европейский Союз |
| Intune | Европейский Союз |
| MyAnalytics | Европейский Союз |
| Планировщик | Европейский Союз |
| Sway | Соединенные Штаты |
| Yammer | Европейский Союз  |
| Службы OneNote | Европейский Союз |
| Stream | Европейский Союз |
| Доска | Европейский Союз |
| Forms | Европейский Союз |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="greenland"></a>Гренландия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="grenada"></a>Гренада
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="guadeloupe"></a>Гваделупа
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="guam"></a>Гуам
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online и Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="guatemala"></a>Гватемала
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="guernsey"></a>Гернси
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="guinea"></a>Гвинея
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="guinea-bissau"></a>Гвинея-Бисау
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="guyana"></a>Гайана
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 3 — Северная и Южная Америка |
| OneDrive для бизнеса | Глобальный географический регион 3 — Северная и Южная Америка |
| SharePoint Online | Глобальный географический регион 3 — Северная и Южная Америка |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Глобальный географический регион 3 — Северная и Южная Америка |
| EOP | Глобальный географический регион 3 — Северная и Южная Америка |
| Intune | Соединенные Штаты |
| MyAnalytics | Глобальный географический регион 3 — Северная и Южная Америка |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 3 — Северная и Южная Америка |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="haiti"></a>Гаити
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online &amp; Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="heard-and-mcdonald-islands"></a>О-ва Херд и Макдональд
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online &amp; Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="herzegovina"></a>Герцеговина
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="holy-see-vatican-city-state"></a>Папский Престол (Ватикан)
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="honduras"></a>Гондурас
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="hong-kong-sar"></a>Гонконг, САР
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online &amp; Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="hungary"></a>Венгрия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Европейский Союз |
| OneDrive для бизнеса | Европейский Союз |
| SharePoint Online | Европейский Союз |
| Skype для бизнеса | Европейский Союз |
| Microsoft Teams | Европейский Союз |
| Office Online и Mobile | Европейский Союз |
| EOP | Европейский Союз |
| Intune | Европейский Союз |
| MyAnalytics | Европейский Союз |
| Планировщик | Европейский Союз |
| Sway | Соединенные Штаты |
| Yammer | Европейский Союз  |
| Службы OneNote | Европейский Союз |
| Stream | Европейский Союз |
| Доска | Европейский Союз |
| Forms | Европейский Союз |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="iceland"></a>Исландия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="india"></a>Индия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Индия |
| OneDrive для бизнеса | Индия |
| SharePoint Online | Индия |
| Skype для бизнеса | Индия |
| Microsoft Teams | Индия |
| Office Online и Mobile | Индия |
| EOP | Индия |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Индия |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Индия |
| Stream | Индия |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="indonesia"></a>Индонезия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online &amp; Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="iraq"></a>Ирак
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="ireland"></a>Ирландия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Европейский Союз |
| OneDrive для бизнеса | Европейский Союз |
| SharePoint Online | Европейский Союз |
| Skype для бизнеса | Европейский Союз |
| Microsoft Teams | Европейский Союз |
| Office Online и Mobile | Европейский Союз |
| EOP | Европейский Союз |
| Intune | Европейский Союз |
| MyAnalytics | Европейский Союз |
| Планировщик | Европейский Союз |
| Sway | Соединенные Штаты |
| Yammer | Европейский Союз  |
| Службы OneNote | Европейский Союз |
| Stream | Европейский Союз |
| Доска | Европейский Союз |
| Forms | Европейский Союз |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="isle-of-man"></a>О-в Мэн
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="israel"></a>Израиль
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="italy"></a>Италия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Европейский Союз |
| OneDrive для бизнеса | Европейский Союз |
| SharePoint Online | Европейский Союз |
| Skype для бизнеса | Европейский Союз |
| Microsoft Teams | Европейский Союз |
| Office Online и Mobile | Европейский Союз |
| EOP | Европейский Союз |
| Intune | Европейский Союз |
| MyAnalytics | Европейский Союз |
| Планировщик | Европейский Союз |
| Sway | Соединенные Штаты |
| Yammer | Европейский Союз  |
| Службы OneNote | Европейский Союз |
| Stream | Европейский Союз |
| Доска | Европейский Союз |
| Forms | Европейский Союз |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="jamaica"></a>Ямайка
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online &amp; Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="japan"></a>Япония
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Япония |
| OneDrive для бизнеса | Япония |
| SharePoint Online | Япония |
| Skype для бизнеса | Япония |
| Microsoft Teams | Япония |
| Office Online и Mobile | Япония |
| EOP | Япония |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Япония |
| Планировщик | Япония |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Япония |
| Stream | Япония |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="jersey"></a>Джерси
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer |  Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="jordan"></a>Иордания
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="kazakhstan"></a>Казахстан
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="kenya"></a>Кения
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="kiribati"></a>Кирибати
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online &amp; Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="south-korea"></a>Южная Корея
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Южная Корея |
| OneDrive для бизнеса | Южная Корея |
| SharePoint Online | Южная Корея |
| Skype для бизнеса | Южная Корея |
| Microsoft Teams | Южная Корея |
| Office Online и Mobile | Южная Корея |
| EOP | Южная Корея |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Южная Корея |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Южная Корея |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="kosovo"></a>Косово
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="kuwait"></a>Кувейт
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="kyrgyzstan"></a>Киргизия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="laos"></a>Лаос
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online &amp; Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="latvia"></a>Латвия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Европейский Союз |
| OneDrive для бизнеса | Европейский Союз |
| SharePoint Online | Европейский Союз |
| Skype для бизнеса | Европейский Союз |
| Microsoft Teams | Европейский Союз |
| Office Online и Mobile | Европейский Союз |
| EOP | Европейский Союз |
| Intune | Европейский Союз |
| MyAnalytics | Европейский Союз |
| Планировщик | Европейский Союз |
| Sway | Соединенные Штаты |
| Yammer | Европейский Союз  |
| Службы OneNote | Европейский Союз |
| Stream | Европейский Союз |
| Доска | Европейский Союз |
| Forms | Европейский Союз |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="lebanon"></a>Ливан
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer |  Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="lesotho"></a>Лесото
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="liberia"></a>Либерия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="libya"></a>Ливия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="liechtenstein"></a>Лихтенштейн
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Швейцария |
| OneDrive для бизнеса | Швейцария |
| SharePoint Online | Швейцария |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Швейцария |
| Office Online и Mobile | Швейцария |
| EOP | Швейцария |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Швейцария |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Швейцария |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="lithuania"></a>Литва
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Европейский Союз |
| OneDrive для бизнеса | Европейский Союз |
| SharePoint Online | Европейский Союз |
| Skype для бизнеса | Европейский Союз |
| Microsoft Teams | Европейский Союз |
| Office Online и Mobile | Европейский Союз |
| EOP | Европейский Союз |
| Intune | Европейский Союз |
| MyAnalytics | Европейский Союз |
| Планировщик | Европейский Союз |
| Sway | Соединенные Штаты |
| Yammer | Европейский Союз  |
| Службы OneNote | Европейский Союз |
| Stream | Европейский Союз |
| Доска | Европейский Союз |
| Forms | Европейский Союз |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="luxembourg"></a>Люксембург
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Европейский Союз |
| OneDrive для бизнеса | Европейский Союз |
| SharePoint Online | Европейский Союз |
| Skype для бизнеса | Европейский Союз |
| Microsoft Teams | Европейский Союз |
| Office Online и Mobile | Европейский Союз |
| EOP | Европейский Союз |
| Intune | Европейский Союз |
| MyAnalytics | Европейский Союз |
| Планировщик | Европейский Союз |
| Sway | Соединенные Штаты |
| Yammer | Европейский Союз  |
| Службы OneNote | Европейский Союз |
| Stream | Европейский Союз |
| Доска | Европейский Союз |
| Forms | Европейский Союз |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="madagascar"></a>Мадагаскар
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="malawi"></a>Малави
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="malaysia"></a>Малайзия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online &amp; Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="maldives"></a>Мальдивы
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online &amp; Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="mali"></a>Мали
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="malta"></a>Мальта
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Европейский Союз |
| OneDrive для бизнеса | Европейский Союз |
| SharePoint Online | Европейский Союз |
| Skype для бизнеса | Европейский Союз |
| Microsoft Teams | Европейский Союз |
| Office Online и Mobile | Европейский Союз |
| EOP | Европейский Союз |
| Intune | Европейский Союз |
| MyAnalytics | Европейский Союз |
| Планировщик | Европейский Союз |
| Sway | Соединенные Штаты |
| Yammer | Европейский Союз  |
| Службы OneNote | Европейский Союз |
| Stream | Европейский Союз |
| Доска | Европейский Союз |
| Forms | Европейский Союз |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="marshall-islands"></a>Маршалловы Острова
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online и Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="martinique"></a>Мартиника
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="mauritania"></a>Мавритания
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="mauritius"></a>Маврикий
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="mayotte"></a>Майотта
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="mexico"></a>Мексика
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="moldova"></a>Молдова
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="monaco"></a>Монако
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="mongolia"></a>Монголия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online и Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="montenegro"></a>Черногория
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="montserrat"></a>Монтсеррат
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="morocco"></a>Марокко
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="mozambique"></a>Мозамбик
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="myanmar"></a>Мьянма
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online &amp; Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="namibia"></a>Намибия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="nauru"></a>Науру
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online &amp; Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="nepal"></a>Непал
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online и Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="netherlands"></a>Нидерланды
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Европейский Союз |
| OneDrive для бизнеса | Европейский Союз |
| SharePoint Online | Европейский Союз |
| Skype для бизнеса | Европейский Союз |
| Microsoft Teams | Европейский Союз |
| Office Online и Mobile | Европейский Союз |
| EOP | Европейский Союз |
| Intune | Европейский Союз |
| MyAnalytics | Европейский Союз |
| Планировщик | Европейский Союз |
| Sway | Соединенные Штаты |
| Yammer | Европейский Союз  |
| Службы OneNote | Европейский Союз |
| Stream | Европейский Союз |
| Доска | Европейский Союз |
| Forms | Европейский Союз |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="netherlands-antilles"></a>Нидерландские Антильские острова
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online &amp; Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="new-caledonia"></a>Новая Каледония
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online &amp; Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="new-zealand"></a>Новая Зеландия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Австралия |
| OneDrive для бизнеса | Австралия |
| SharePoint Online | Австралия |
| Skype для бизнеса | Австралия |
| Microsoft Teams | Австралия |
| Office Online и Mobile | Австралия |
| EOP | Австралия |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Австралия |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Австралия |
| Stream | Австралия |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="nicaragua"></a>Никарагуа
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="niger"></a>Нигер
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="nigeria"></a>Нигерия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="niue"></a>Ниуэ
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online и Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="norfolk-island"></a>О-в Норфолк
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online и Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="northern-mariana-islands"></a>Северные Марианские о-ва
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online и Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="norway"></a>Норвегия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Норвегия |
| OneDrive для бизнеса | Норвегия |
| SharePoint Online | Норвегия |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Норвегия |
| Office Online и Mobile | Норвегия |
| EOP | Норвегия |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Норвегия |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Норвегия |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="oman"></a>Оман
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="pakistan"></a>Пакистан
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="palau"></a>Палау
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online &amp; Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="palestinian-authority"></a>Палестинская автономия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="panama"></a>Панама
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online &amp; Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="papua-new-guinea"></a>Папуа — Новая Гвинея
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online и Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="paraguay"></a>Парагвай
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 3 — Северная и Южная Америка |
| OneDrive для бизнеса | Глобальный географический регион 3 — Северная и Южная Америка |
| SharePoint Online | Глобальный географический регион 3 — Северная и Южная Америка |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Глобальный географический регион 3 — Северная и Южная Америка |
| EOP | Глобальный географический регион 3 — Северная и Южная Америка |
| Intune | Соединенные Штаты |
| MyAnalytics | Глобальный географический регион 3 — Северная и Южная Америка |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 3 — Северная и Южная Америка |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="peru"></a>Перу
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 3 — Северная и Южная Америка |
| OneDrive для бизнеса | Глобальный географический регион 3 — Северная и Южная Америка |
| SharePoint Online | Глобальный географический регион 3 — Северная и Южная Америка |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Глобальный географический регион 3 — Северная и Южная Америка |
| EOP | Глобальный географический регион 3 — Северная и Южная Америка |
| Intune | Соединенные Штаты |
| MyAnalytics | Глобальный географический регион 3 — Северная и Южная Америка |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 3 — Северная и Южная Америка |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="philippines"></a>Филиппины
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online и Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="pitcairn"></a>О-в Питкэрн
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online &amp; Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="poland"></a>Польша
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Европейский Союз |
| OneDrive для бизнеса | Европейский Союз |
| SharePoint Online | Европейский Союз |
| Skype для бизнеса | Европейский Союз |
| Microsoft Teams | Европейский Союз |
| Office Online и Mobile | Европейский Союз |
| EOP | Европейский Союз |
| Intune | Европейский Союз |
| MyAnalytics | Европейский Союз |
| Планировщик | Европейский Союз |
| Sway | Соединенные Штаты |
| Yammer | Европейский Союз  |
| Службы OneNote | Европейский Союз |
| Stream | Европейский Союз |
| Доска | Европейский Союз |
| Forms | Европейский Союз |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="portugal"></a>Португалия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Европейский Союз |
| OneDrive для бизнеса | Европейский Союз |
| SharePoint Online | Европейский Союз |
| Skype для бизнеса | Европейский Союз |
| Microsoft Teams | Европейский Союз |
| Office Online и Mobile | Европейский Союз |
| EOP | Европейский Союз |
| Intune | Европейский Союз |
| MyAnalytics | Европейский Союз |
| Планировщик | Европейский Союз |
| Sway | Соединенные Штаты |
| Yammer | Европейский Союз  |
| Службы OneNote | Европейский Союз |
| Stream | Европейский Союз |
| Доска | Европейский Союз |
| Forms | Европейский Союз |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="puerto-rico"></a>Пуэрто-Рико
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="qatar"></a>Катар
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="republic-of-macedonia"></a>Республика Северная Македония
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="runion"></a>Реюньон
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="romania"></a>Румыния
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Европейский Союз |
| OneDrive для бизнеса | Европейский Союз |
| SharePoint Online | Европейский Союз |
| Skype для бизнеса | Европейский Союз |
| Microsoft Teams | Европейский Союз |
| Office Online и Mobile | Европейский Союз |
| EOP | Европейский Союз |
| Intune | Европейский Союз |
| MyAnalytics | Европейский Союз |
| Планировщик | Европейский Союз |
| Sway | Соединенные Штаты |
| Yammer | Европейский Союз  |
| Службы OneNote | Европейский Союз |
| Stream | Европейский Союз |
| Доска | Европейский Союз |
| Forms | Европейский Союз |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="russian-federation"></a>Российская Федерация
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="rwanda"></a>Руанда
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="saint-helena"></a>О-в Св. Елены
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="saint-kitts-and-nevis"></a>Сент-Китс и Невис
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="saint-lucia"></a>Сент-Люсия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="saint-martin"></a>Сен-Мартен
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="saint-pierre-and-miquelon"></a>Сен-Пьер и Микелон
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="saint-vincent-and-the-grenadines"></a>Сент-Винсент и Гренадины
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="saint-barthlemy"></a>Сен-Бартелеми
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="samoa"></a>Самоа
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online и Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="san-marino"></a>Сан-Марино
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="sao-tome-and-principe"></a>Сан-Томе и Принсипи
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="saudi-arabia"></a>Саудовская Аравия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="senegal"></a>Сенегал
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="serbia"></a>Сербия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="seychelles"></a>Сейшельские Острова
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="sierra-leone"></a>Сьерра-Леоне
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="singapore"></a>Сингапур
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online и Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="sint-maarten"></a>Синт-Мартен
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="slovakia"></a>Словакия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Европейский Союз |
| OneDrive для бизнеса | Европейский Союз |
| SharePoint Online | Европейский Союз |
| Skype для бизнеса | Европейский Союз |
| Microsoft Teams | Европейский Союз |
| Office Online и Mobile | Европейский Союз |
| EOP | Европейский Союз |
| Intune | Европейский Союз |
| MyAnalytics | Европейский Союз |
| Планировщик | Европейский Союз |
| Sway | Соединенные Штаты |
| Yammer | Европейский Союз  |
| Службы OneNote | Европейский Союз |
| Stream | Европейский Союз |
| Доска | Европейский Союз |
| Forms | Европейский Союз |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="slovenia"></a>Словения
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Европейский Союз |
| OneDrive для бизнеса | Европейский Союз |
| SharePoint Online | Европейский Союз |
| Skype для бизнеса | Европейский Союз |
| Microsoft Teams | Европейский Союз |
| Office Online и Mobile | Европейский Союз |
| EOP | Европейский Союз |
| Intune | Европейский Союз |
| MyAnalytics | Европейский Союз |
| Планировщик | Европейский Союз |
| Sway | Соединенные Штаты |
| Yammer | Европейский Союз  |
| Службы OneNote | Европейский Союз |
| Stream | Европейский Союз |
| Доска | Европейский Союз |
| Forms | Европейский Союз |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="solomon-islands"></a>Соломоновы Острова
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online &amp; Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="somalia"></a>Сомали
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="south-africa"></a>Южная Африка
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Южная Африка |
| OneDrive для бизнеса | Южная Африка |
| SharePoint Online | Южная Африка |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Южная Африка |
| Office Online и Mobile | Южная Африка |
| EOP | Южная Африка |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Южная Африка |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Южная Африка |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="south-georgia-and-the-south-sandwich-islands"></a>Южная Джорджия и Южные Сандвичевы Острова
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="spain"></a>Испания
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Европейский Союз |
| OneDrive для бизнеса | Европейский Союз |
| SharePoint Online | Европейский Союз |
| Skype для бизнеса | Европейский Союз |
| Microsoft Teams | Европейский Союз |
| Office Online и Mobile | Европейский Союз |
| EOP | Европейский Союз |
| Intune | Европейский Союз |
| MyAnalytics | Европейский Союз |
| Планировщик | Европейский Союз |
| Sway | Соединенные Штаты |
| Yammer | Европейский Союз  |
| Службы OneNote | Европейский Союз |
| Stream | Европейский Союз |
| Доска | Европейский Союз |
| Forms | Европейский Союз |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="sri-lanka"></a>Шри-Ланка
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online и Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="suriname"></a>Суринам
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="svalbard-and-jan-mayen-islands"></a>Шпицберген и Ян-Майен
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="swaziland"></a>Свазиленд
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="sweden"></a>Швеция
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Европейский Союз |
| OneDrive для бизнеса | Европейский Союз |
| SharePoint Online | Европейский Союз |
| Skype для бизнеса | Европейский Союз |
| Microsoft Teams | Европейский Союз |
| Office Online и Mobile | Европейский Союз |
| EOP | Европейский Союз |
| Intune | Европейский Союз |
| MyAnalytics | Европейский Союз |
| Планировщик | Европейский Союз |
| Sway | Соединенные Штаты |
| Yammer | Европейский Союз  |
| Службы OneNote | Европейский Союз |
| Stream | Европейский Союз |
| Доска | Европейский Союз |
| Forms | Европейский Союз |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="switzerland"></a>Швейцария
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Швейцария |
| OneDrive для бизнеса | Швейцария |
| SharePoint Online | Швейцария |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Швейцария |
| Office Online и Mobile | Швейцария |
| EOP | Швейцария |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Швейцария |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Швейцария |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Глобальный географический регион 1 — EMEA |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="taiwan"></a>Тайвань
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online &amp; Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="tajikistan"></a>Таджикистан
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="united-republic-of-tanzania"></a>Объединенная Республика Танзания
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="thailand"></a>Таиланд
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online &amp; Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="timor-leste"></a>Тимор-Лесте
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online &amp; Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="togo"></a>Того
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="tokelau"></a>Токелау
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online &amp; Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="tonga"></a>Тонга
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online &amp; Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="trinidad-and-tobago"></a>Тринидад и Тобаго
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="tunisia"></a>Тунис
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="turkey"></a>Турция
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="turkmenistan"></a>Туркменистан
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="turks-and-caicos-islands"></a>О-ва Теркс и Кайкос
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="tuvalu"></a>Тувалу
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online и Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="us-virgin-islands"></a>Виргинские о-ва (США)
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="uganda"></a>Уганда
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="ukraine"></a>Украина
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online &amp; Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="united-arab-emirates"></a>Объединенные Арабские Эмираты
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Объединенные Арабские Эмираты |
| OneDrive для бизнеса | Объединенные Арабские Эмираты |
| SharePoint Online | Объединенные Арабские Эмираты |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Объединенные Арабские Эмираты |
| Office Online и Mobile | Объединенные Арабские Эмираты |
| EOP | Объединенные Арабские Эмираты |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Объединенные Арабские Эмираты |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Объединенные Арабские Эмираты |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="united-kingdom"></a>Соединенное Королевство
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенное Королевство |
| OneDrive для бизнеса | Соединенное Королевство |
| SharePoint Online | Соединенное Королевство |
| Skype для бизнеса | Соединенное Королевство |
| Microsoft Teams | Соединенное Королевство |
| Office Online и Mobile | Соединенное Королевство |
| EOP | Соединенное Королевство |
| Intune | Европейский Союз |
| MyAnalytics | Соединенное Королевство |
| Планировщик | Европейский Союз |
| Sway | Соединенные Штаты |
| Yammer | Европейский Союз |
| Службы OneNote | Соединенное Королевство |
| Stream | Соединенное Королевство |
| Доска | Европейский Союз |
| Forms | Европейский Союз |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="united-states"></a>Соединенные Штаты
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online &amp; Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="uruguay"></a>Уругвай
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 3 — Северная и Южная Америка |
| OneDrive для бизнеса | Глобальный географический регион 3 — Северная и Южная Америка |
| SharePoint Online | Глобальный географический регион 3 — Северная и Южная Америка |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online &amp; Mobile | Глобальный географический регион 3 — Северная и Южная Америка |
| EOP | Глобальный географический регион 3 — Северная и Южная Америка |
| Intune | Соединенные Штаты |
| MyAnalytics | Глобальный географический регион 3 — Северная и Южная Америка |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 3 — Северная и Южная Америка |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="us-minor-outlying-islands"></a>Внешние малые о-ва (США)
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online &amp; Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="uzbekistan"></a>Узбекистан
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="vanuatu"></a>Вануату
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online и Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="venezuela"></a>Венесуэла
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Соединенные Штаты |
| OneDrive для бизнеса | Соединенные Штаты |
| SharePoint Online | Соединенные Штаты |
| Skype для бизнеса | Соединенные Штаты |
| Microsoft Teams | Соединенные Штаты |
| Office Online и Mobile | Соединенные Штаты |
| EOP | Соединенные Штаты |
| Intune | Соединенные Штаты |
| MyAnalytics | Соединенные Штаты |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Соединенные Штаты |
| Stream | Соединенные Штаты |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="vietnam"></a>Вьетнам
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online и Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="wallis-and-futuna-islands"></a>Острова Уоллис и Футуна
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| OneDrive для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| SharePoint Online | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Skype для бизнеса | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Microsoft Teams | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Office Online &amp; Mobile | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| EOP | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Intune | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| MyAnalytics | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Планировщик | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Sway | Соединенные Штаты |
| Yammer | Соединенные Штаты |
| Службы OneNote | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Stream | Глобальный географический регион 2 — Азиатско-Тихоокеанский регион |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="western-sahara"></a>Западная Сахара
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="yemen"></a>Йемен
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Глобальный географический регион 1 — EMEA |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Глобальный географический регион 1 — EMEA |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="zambia"></a>Замбия
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>

## <a name="zimbabwe"></a>Зимбабве
<details><summary>Щелкните, чтобы развернуть</summary><p>

| Служба | Расположение |
| --- | --- |
| Exchange Online | Глобальный географический регион 1 — EMEA |
| OneDrive для бизнеса | Глобальный географический регион 1 — EMEA |
| SharePoint Online | Глобальный географический регион 1 — EMEA |
| Skype для бизнеса | Глобальный географический регион 1 — EMEA |
| Microsoft Teams | Глобальный географический регион 1 — EMEA |
| Office Online и Mobile | Глобальный географический регион 1 — EMEA |
| EOP | Глобальный географический регион 1 — EMEA |
| Intune | Глобальный географический регион 1 — EMEA |
| MyAnalytics | Глобальный географический регион 1 — EMEA |
| Планировщик | Соединенные Штаты |
| Sway | Соединенные Штаты |
| Yammer | Глобальный географический регион 1 — EMEA |
| Службы OneNote | Глобальный географический регион 1 — EMEA |
| Stream | Глобальный географический регион 1 — EMEA |
| Доска | Соединенные Штаты |
| Forms | Соединенные Штаты |
| Рабочая аналитика | Соединенные Штаты |
</p></details>
